...
-- Create Table DislikedMovie(UserPassword varchar(255), UserName varchar(255), ID varchar(9), FOREIGN KEY (UserPassword, UserName) REFERENCES ProgramUser(UserPassword, UserName), FOREIGN KEY (ID) REFERENCES movie(ID));

-- the movie csv has a list of actors/ writers / directors that we will pull from to create this
-- relationship table
-- note: a small probability (1.47%) of the actors have the same name. they will just be removed from the
-- data set 
...
Where P.City like ('%' + @A.PlaceOfBirth + '%');
