...
-- the actor ID does not need to be a key becuase the user only has one favorite
...
-- this one is going to be super intresting because we are joining two diffrent data sets
-- first step is to look through and connect the place and where the people were born in
-- second step is to create a new location table that only have the places where people are born becuase 
...
-- note: the born in is diffrent from contry ex born in has USA where contry has US so we need to figute out how to do this
